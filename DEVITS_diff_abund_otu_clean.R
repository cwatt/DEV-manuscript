#########################################
#DEV ITS differential abundance OTU level
#Cassandra Wattenburger

#NOTES: 
#This code analyzes differential abundance at the OTU level using DESeq2
#This script uses files created in "DEVITS_deseq2_prep_clean.R"
...
values <- as.numeric(c(66,47,62,49,8,2,4,4,14,8,7,7)) 
...
#total OTUs label
label.b <- data.frame(Phylum = 5, log2FoldChange = 5, 
...
ggsave("DEVITS_diffabund_point_bb.tiff", plot = b.point, scale = 1, width=7, height=4, units="in")
...
x.rs = sort(x.rs, TRUE)
rs.all$Phylum = factor(as.character(rs.all$Phylum), levels=names(x.rs)) #this line messes up the phylum column for some reason, need to troubleshoot

#total OTUs label
label.rs <- data.frame(Phylum = 3, log2FoldChange = 4, 
...
rs.point <- ggplot(rs.all, aes(x=Phylum, y=log2FoldChange, color=Class)) + 
  ...
  facet_grid(Timepoint~.) +
  geom_hline(yintercept=0, color="grey69", linetype=2) +
  geom_text(data = label.rs, aes(Phylum, log2FoldChange, label=label), inherit.aes=FALSE, size=2) +
  labs(title="", 
...
ggsave("DEVITS_diffabund_point_rsrs.tiff", plot = rs.point, scale = 1, width=7, height=4, units="in")
...
#total OTUs label
label.rp <- data.frame(Phylum = 3, log2FoldChange = 5, 
...
ggsave("DEVITS_diffabund_point_rprp.tiff", plot = rp.point, scale = 1, width=7, height=4, units="in")
...
#total OTUs label
label.brs <- data.frame(Phylum = 3, log2FoldChange = 8, 
...
ggsave("DEVITS_diffabund_point_brs.tiff", plot = brs.point, scale = 1, width=9.7, height=7, units="in")
...
#total OTUs label
label.brp <- data.frame(Phylum = 3.5, log2FoldChange = 10, 
...
ggsave("DEVITS_diffabund_point_brp.tiff", plot = brp.point, scale = 1, width=9.7, height=7, units="in")
